...
class App extends React.Component {
        ...
        const render = () => {
            if (categories === 'index') {
                dataList = DATA_NAV.filter(v => v.categories !== 'index').reduce((prev, cur) => {
                    prev.push(
                        ...DATA_ARTICLE[cur.categories].map(v => ({
    ...
    render() {
        const {openNav, navIndex, content} = this.state;
        return (
            <div>
                <nav className="nav">
                    <div className="inner">
                        <div className="wrap">
                            ...
                            <span
                                className={classnames('navbar', {active: openNav})}
                                onClick={this.openNav}
                            >
                                {[0, 1, 2, 3].map(v => (
                                    <span key={v} className="bar" />
                                ))}
                            </span>
                            <ul style={{height: openNav ? DATA_NAV.length * 40 + 20 : 0}}>
                                {DATA_NAV.map((v, i) => (
                                    <li
                                        key={`${v.categories + v.name}`}
                                        className={classnames({active: navIndex === i})}
                                    >
                ...
                <footer className="footer">
                    <div className="outer">
                        <div className="wrap">
                            <span>
                                © 2014-
                                {new Date().getFullYear()} Shuoshubao.com 版权所有{' '}
                                {DATA_META.registration}
                            </span>
